...
2. Для поиска по конкретному источнику добавлять к запросу `site:домен`, а сам
   термин заключать в кавычки. Даже с Tavily проверять домен каждого результата:
   оператор `site:` — подсказка поиску, а не граница безопасности.
...
3. Выполнить три обязательных запроса с точными `startDate`/`endDate`,
   `topic=news`, `searchDepth=advanced` и восемью результатами: глобальные
   релизы; Азия/open weights; независимые измерения, цены и деловой контекст.
   После проверки покрытия разрешён один дополнительный ремонтный запрос по
   отсутствующему треку или точным именам кандидатов. Не делать отдельный запрос
   на каждый домен.
