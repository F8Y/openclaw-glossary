...
Реестр внешних источников и правила их использования. Это не замена
`memory/knowledge/`: локальные глоссарии остаются проверенной базой, а веб —
актуальным резервным слоем и источником новостей.

Читать этот файл перед внешним поиском термина и перед подготовкой дайджеста.
...
3. `web_search` использовать для обнаружения страницы, затем открывать выбранный
   URL через `web_fetch`. Не выдавать сниппет поисковика за полный первоисточник.
4. Ссылаться на конкретную страницу, а не на главную страницу сайта и не на
   страницу поисковой выдачи.
5. Английское определение переводить на русский, сохраняя исходную
   расшифровку аббревиатуры и важные термины на английском.
6. Если источники расходятся, не склеивать определения. Показать расхождение,
...
1. Определить точный период дайджеста и искать события этого периода. Отличать
   дату события от даты публикации или обновления страницы.
2. Выполнить несколько поисков по группам источников A, затем один-два поиска по
   B и C. Не полагаться на одну общую выдачу.
3. Для кандидата открыть первичный источник через `web_fetch`. Удалить дубликаты,
   где несколько СМИ пересказывают один анонс.
4. Включать новость только при наличии конкретного изменения: модель, API,
   цена, лицензия, доступность, benchmark, крупная сделка или регулирование.
5. Каждый пункт снабжать ссылкой на первичный источник. Существенное спорное или
   сравнительное утверждение по возможности подтверждать независимым источником.
6. Отмечать статус: `GA`, `preview`, `research`, `open weights` или
