...
2. Выполнить не более трёх запросов: по первичным источникам A, затем по
   независимым источникам B и контексту C. Не делать отдельный запрос на каждый
   сайт без необходимости.
3. Сначала составить короткий список кандидатов, затем открыть через `web_fetch`
   только выбранные страницы — по одной попытке на URL. Сбой одной страницы не
   должен перезапускать весь поиск. Удалить дубликаты, где несколько СМИ
   пересказывают один анонс.
