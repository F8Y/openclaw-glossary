...
Бот использует `dmPolicy: "pairing"`. Пользователи, уже записанные в
`allowFrom`, работают как раньше. Неизвестный пользователь не попадает к
агенту, инструментам или LLM: бот отвечает ему только одноразовым кодом
подключения и Telegram ID, которые можно передать владельцу.

Это не делает бота публичным. До добавления или подтверждения ID запросы
неизвестного пользователя не обрабатываются.
...
1. Новый пользователь открывает бота и отправляет `/start`.
2. Если Telegram только открыл диалог и ответ не появился, нужно отправить
   обычное сообщение, например `Привет`.
3. Бот покажет одноразовый код подключения и Telegram ID. Владелец также
   может увидеть ожидающий запрос на сервере:

```bash
oc pairing list telegram --json
```

Для постоянного GitOps-доступа числовой ID добавляется в оба списка выше,
затем проходит обычный цикл PR → CI → Promote. Одноразовый код подтверждать
необязательно, если ID сразу добавляется декларативно.

Для временного доступа к обычному диалогу код можно подтвердить вручную:

```bash
oc pairing approve telegram <CODE> --notify
```

Подтверждение pairing даёт доступ к личному диалогу, но не добавляет человека
в `commands.allowFrom.telegram` и не делает его владельцем. Полный UI с
командами всё равно оформляется через Git.
